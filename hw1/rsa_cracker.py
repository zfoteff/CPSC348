...
def extended_greatest_common_denominator(a, b):
    ...
    Return (g, x, y) such that
        a*x + b*y = g = greatest_common_denominator(a, b)
    """
    if a == 0:
        return (b, 0, 1)
    else:
...
def encryptRSA(e, n , text):
    """
    Encrypt a plaintext message to ciphertext using RSA encyption algorithm
...
def decryptRSA(d, n, text):
    """
    Decrypt a ciphertext message to plaintext using RSA decryption algorithm
...
def crackRsaPrivateKey(e, n):
    ...
    # since n is the product of two primes, we know its only factors are those
    # two primes if this is not the case, n is invalid (i.e., not the product of
    # two primes)
...
def printAlgorithmResults (test_case, key_size, plaintext, ciphertext, e, n, d):
    """
    Test and output the results of
    """
    brute_force_time = -1
...
def main():
    ...
    e_list =    [7,3,17,11,7,1319,20293957,293210035,5292724727,65537,65537,
                65537,65537]
    n_list =    [33,55,77,143,527,1717,24899297,660416987,8093802337,
                26205577898256853,221000191580019839159,23800004200970185016117,
                1769685722799684125246893]
    ...
    # Repeat the for all test cases
    for case in range (0, len(key_sizes)):
